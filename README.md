...
| `upload_timeout`     | Optional     | `5`                | Per-file upload timeout in **minutes**. If exceeded, the upload is retried up to 3 times; if all attempts fail, the file is skipped. Set to `0` to disable the time limit. Note: even when `upload_timeout=0`, the stall guard remains active—uploads slower than 1 KB/s for 60 consecutive seconds will still trigger retries.                                                                                                  |
